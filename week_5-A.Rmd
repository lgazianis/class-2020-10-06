...
```{r q1}
election_data <- read_csv("data_538/election-forecasts-2020/presidential_ev_probabilities_2020.csv",
                          col_types = cols(
  cycle = col_double(),
...
ggplot(data = election_data, mapping = aes(x = total_ev, y = evprob_chal)) +
  geom_col(fill = "dodgerblue") + 
  labs(title = "Biden's Projected Electoral Vote Counts",
       x = "Total Electoral Votes",
       y = "Probability") + theme_minimal() +
  geom_vline(xintercept = 270) +
  annotate("text", x = 270, y = .02, label = "victory threshold")
```
...
election_data %>%
  ...
  filter(total_ev > 300) %>%
  summarize(prob_300 = sum(evprob_chal))
```
...
election_data %>%
  ...
  summarize(close_election = sum(evprob_chal))
```
